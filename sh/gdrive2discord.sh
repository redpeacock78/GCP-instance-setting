...
AUTHOR_NAME="$(dir_watch && git log -1 --pretty=format:'%aN' -C ${TARGET_DIR})"
GIT_MESSAGE="$(dir_watch && git log -1 --pretty=format:'%s' -C ${TARGET_DIR})"
GIT_HASH="$(dir_watch && git log -1 --pretty=format:'%h' -C ${TARGEY_DIR})"
TIME_STAMP="$(dir_watch && date -d "$(git log -1 --pretty=format:'%cD' -C ${TARGET_DIR})" +%s)"
